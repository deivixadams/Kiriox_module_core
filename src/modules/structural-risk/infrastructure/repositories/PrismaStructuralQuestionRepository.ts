    ...
    try {
      ...
      return [
        ...
        },
        {
          ...
          source_of_truth: 'public.obligation_graph',
          source_kind: 'table',
          result_type: 'table',
          answer_renderer: 'table',
...
async function buildRiskFocusedGraph(context: ExecutionContext) {
  return buildFragilityPriorityGraph(context);
}
...
async function buildControlFocusedGraph(context: ExecutionContext) {
  ...
  const risks = await prisma.$queryRawUnsafe<Array<{
    ...
    `SELECT
     ...
     FROM public.map_risk_control m
     JOIN public.run_ra_risks r
       ON r.id = m.risk_id
...
async function buildElementFocusedGraph(context: ExecutionContext) {
  ...
  const risks = await prisma.$queryRawUnsafe<Array<{
    ...
    `SELECT
        ...
        r.name as risk_name,
        m.link_strength
     FROM public.map_elements_risk m
     JOIN public.run_ra_risks r
       ON r.id = m.risk_id

...
import type { IQuestionGraphRepository, QuestionGraphElement } from '@/modules/structural-risk/domain/contracts/IQuestionGraphRepository';

const GRAPH_NODE_TYPES = ['ELEMENT', 'RISK', 'CONTROL'];
...
export class PrismaQuestionGraphRepository implements IQuestionGraphRepository {
  async getFullGraph(reinoId?: string): Promise<QuestionGraphElement[]> {
    const nodeParams: unknown[] = [GRAPH_NODE_TYPES];
    ...
    const edgeParams: unknown[] = [GRAPH_NODE_TYPES, GRAPH_NODE_TYPES];
    ...
    return [
      ...nodes.map((node) => ({
        ...
        element_data: {
          ...
          type: node.node_type,
          size: SIZE_MAP[node.node_type] ?? 46,
        },
      })),
      ...edges.map((edge) => ({

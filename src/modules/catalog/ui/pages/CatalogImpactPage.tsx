...
const EMPTY: Omit<ImpactRow, 'catalog_impact_id'> = {
...
export function CatalogImpactPage() {
  ...
  const [form, setForm]                   = useState<Omit<ImpactRow, 'catalog_impact_id'>>(EMPTY);
  ...
  async function load() {
    setLoading(true);
    try {
      const res  = await fetch('/api/catalogo/impacto');
      const data = await res.json() as { items: ImpactRow[] };
      setItems(data.items ?? []);
    } finally {
      setLoading(false);
    }
  ...
  function openEdit(item: ImpactRow) {
    setEditId(item.catalog_impact_id);
    setForm({
      code:          item.code,
      name:          item.name,
  ...
  async function handleDelete(id: number) {
    setSaving(true);
    try {
      await fetch(`/api/catalogo/impacto?id=${id}`, { method: 'DELETE' });
      ...
      {showForm && (
        <div style={{ ...S.card, marginBottom: '1.5rem', padding: '1.25rem 1.5rem' }}>
          <p style={{ margin: '0 0 1rem', fontSize: '0.8rem', fontWeight: 700, color: '#94a3b8' }}>
            {editId ? 'Editar nivel de impacto' : 'Nuevo nivel de impacto'}
          </p>
            ...
            {loading ? (
              ...
              <tr key={item.catalog_impact_id}>
                ...
                <td style={{ ...S.td, textAlign: 'right' }}>
                  {confirmDelete === item.catalog_impact_id ? (
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem' }}>
                      <span style={{ fontSize: '0.72rem', color: '#f87171' }}>¿Confirmar?</span>
                      <button style={S.btnDanger} onClick={() => void handleDelete(item.catalog_impact_id)} disabled={saving}>Sí</button>
